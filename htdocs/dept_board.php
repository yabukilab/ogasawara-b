...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($title) && !empty($content)) {
        // 新規投稿をデータベースに挿入
        $stmt = $db->prepare("INSERT INTO sabposts (title, content, user_id, department) VALUES (:title, :content, :user_id, :department)");
        $stmt->execute(['title' => $title, 'content' => $content, 'user_id' => $user_id, 'department' => $department]);
    }
}

// 特定の学科の投稿を取得するSQLクエリを実行
$specificDepartment = "プロジェクトマネジメント学科";
$allPostsStmt = $db->prepare("SELECT sabposts.*, users.username FROM sabposts JOIN users ON sabposts.user_id = users.id WHERE users.faculty = '社会システム科学部' AND users.department = :department ORDER BY sabposts.created_at DESC");
$allPostsStmt->execute(['department' => $specificDepartment]);
